...
impl SchemeMut for SMScheme {
    ...
    fn read(&mut self, _file: usize, buf: &mut [u8], _offset: u64, _flags: u32) -> Result<usize> {
        ...
        } else { //other commands or just default read

            if buf.len() < 4 {
                return Err(Error::new(InvalidInput, "Buffer is too small"));
            }
            
            //also flags when implemented

            match file {
                _ =>  {
                    //by default, read should return w/ std::process::id() in buf
                }
            }

            if offset > 0 {
                let offset = offset as usize; //pid with offset
                if offset + 4 > self.pid_buffer.len() {
                    return Err(Error::new(UnexpectedEof, "Offset out of bounds"));
                }
                buf[..4].copy_from_slice(&self.pid_buffer[offset..offset + 4]);

                info!("Read PID from pid_buffer at offset {}: {:?}", offset, &buf[..4]);
            } else {
                let pid = std::process::id(); //get current pid
                let pid_bytes = pid.to_ne_bytes(); //convert to bytes
                buf[..4].copy_from_slice(&pid_bytes[..4]);

                info!("Read current PID {:?} into buffer: {:?}", pid, &buf[..4]);
            }
            Ok(4)
        } */
    }
